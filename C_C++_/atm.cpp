...
// public member functions
    // setData function is setting the data into the tivate member variables 
    ...
    void setMobile(string mob_prev, string mob_new){
        ...
        {
            mobile_No = mob_new;
            // and update the new if the old matches
            cout << endl << "Successfully updates mobile no :) ";
            _getch();    // getch is to hold yhe screen (until user press any key)
        }
        else{   // doesn't update if old mobile no doesnt matches
    ...
    // cashwithdraw ffunction is used to withdraw money from atm
    void cashWithDraw(int amount_a){
        if (amount_a > 0 && amount_a < balance) // check entered amount validity
        {
            balance -= amount_a;  // balance = balce - amount_a
            ...
            cout << endl << "Invalid inpuit or insufficient balance :(";
            _getch();
        }
    }
